    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        <Textarea
          ...
          rows={6}
        />
        <div className="text-xs text-dash-t3 text-right">{body.length} chars</div>
      </div>
...
function ArticleRow({ article, clinicId, onEdit }: { article: KnowledgeArticle; clinicId: string; onEdit: (a: KnowledgeArticle) => void }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-dash-border px-4 py-3 hover:bg-dash-surface transition-colors">
      <div className="flex items-center gap-1.5">
        {STATUS_ICONS[article.status as keyof typeof STATUS_ICONS]}
      </div>
      <div className="flex-1 min-w-0">
        <div className="font-medium text-sm text-dash-t1 truncate">{article.title}</div>
        <div className="text-xs text-dash-t3 truncate mt-0.5">{article.body.slice(0, 80)}...</div>
      </div>
      <Badge variant="outline" className="shrink-0 text-xs">{article.category}</Badge>
      <span className="text-xs text-dash-t3 shrink-0">{relativeTime(article.updated_at)}</span>
      ...
      <Dialog open={confirmDel} onOpenChange={setConfirmDel}>
        <DialogContent>
          ...
          <p className="text-sm text-dash-t3">Delete "{article.title}"? This cannot be undone.</p>
          <DialogFooter>
...
function ImportUrlTab({ clinicId }: { clinicId: string }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-dash-t3">Enter your website URL to import content automatically.</p>
      ...
      {isWorking && (
        <div className="flex items-center gap-2 text-sm text-dash-t3">
  ...
  return (
    <Tabs defaultValue="file">
      ...
      <TabsContent value="file" className="mt-4">
        <div
          className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-dash-border p-10 text-center cursor-pointer hover:border-dash-blue/50 transition-colors"
          onClick={() => fileRef.current?.click()}
        >
          <Upload className="h-8 w-8 text-dash-t3 mb-2" />
          <div className="font-medium text-sm text-dash-t1">Drop files here or click to browse</div>
          <div className="text-xs text-dash-t3 mt-1">PDF, DOCX, TXT — up to 10MB</div>
      ...
      <TabsContent value="template" className="mt-4">
        <div className="grid gap-2">
          {['Standard Dental FAQ Pack', 'Med Spa Policies Template', 'Business Hours Template', 'Payment & Insurance FAQ'].map((t) => (
            <div key={t} className="flex items-center justify-between rounded-lg border border-dash-border px-4 py-3">
              <span className="text-sm font-medium text-dash-t1">{t}</span>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-extrabold text-dash-t1">Knowledge Base</h1>
          <p className="text-sm text-dash-t3 mt-0.5">
      ...
      <div className="flex gap-4">
        ...
        <div className="w-52 shrink-0 space-y-1">
          <button
            ...
            className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors ${
              activeCategory === 'All' ? 'bg-dash-blue-bg text-dash-blue' : 'hover:bg-dash-surface text-dash-t3'
            }`}
          >
          ...
          {CATEGORIES.filter((c) => categoryCounts[c]).map((cat) => (
            <button
              ...
              className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors ${
                activeCategory === cat ? 'bg-dash-blue-bg text-dash-blue' : 'hover:bg-dash-surface text-dash-t3'
              }`}
            >
              <span>{cat}</span>
        ...
        <div className="flex-1 space-y-4">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-dash-t3" />
            <Input
              value={search}
