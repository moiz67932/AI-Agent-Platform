...
function AddNumberDialog({ onClose }: { onClose: () => void }) {
  ...
  const search = async () => {
    ...
    setSearchResults([]);
    try {
      const params = new URLSearchParams({ country: 'US', _t: Date.now().toString() });
      if (areaCode.trim()) params.set('area_code', areaCode.trim());
      const res = await api.get(`/api/numbers/search?${params}`);
      setSearchResults((res as { data: AvailableNumber[] }).data || []);
    } catch (err) {
      toast({
        title: 'Search failed',
        description: isNetworkError(err) ? 'Server unavailable — please try again shortly' : undefined,
        variant: 'destructive',
      });
    } finally {
      setSearching(false);
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        ...
        {/* Tab switcher */}
        <div className="flex gap-2 border-b border-dash-border pb-2">
          <button
            className={`text-sm px-3 py-1 rounded-md transition-colors ${tab === 'search' ? 'bg-dash-blue text-white' : 'text-dash-t3 hover:text-dash-t1'}`}
            onClick={() => setTab('search')}
          >
            Search Available
          </button>
          <button
            className={`text-sm px-3 py-1 rounded-md transition-colors ${tab === 'manual' ? 'bg-dash-blue text-white' : 'text-dash-t3 hover:text-dash-t1'}`}
          ...
          {tab === 'search' ? (
            <>
              <div className="flex gap-2">
                <div className="flex-1 space-y-1">
                  <Label>Area Code <span className="text-dash-t3">(optional)</span></Label>
                  <Input
                    value={areaCode}
              ...
              {searchResults.length > 0 && (
                <div className="max-h-48 overflow-y-auto border border-dash-border rounded-md divide-y divide-dash-border">
                  {searchResults.map((n) => (
                    <button
                      key={n.phoneNumber}
                      className={`w-full text-left px-3 py-2 text-sm hover:bg-dash-surface transition-colors flex justify-between items-center ${selected === n.phoneNumber ? 'bg-dash-blue-bg' : ''}`}
                      onClick={() => setSelected(n.phoneNumber)}
                    >
                      <span className="font-mono font-medium text-dash-t1">{n.friendlyName}</span>
                      {n.locality && <span className="text-xs text-dash-t3">{n.locality}, {n.region}</span>}
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-1.5">
              ...
              <div className="rounded-lg border border-dash-border bg-dash-surface p-3 text-xs text-dash-t3 space-y-1 mt-2">
                ...
                <p>3. Enter the number here to track it in Foyer</p>
              </div>
            </div>
          )}

          <div className="space-y-1.5">
            <Label>Label <span className="text-dash-t3">(optional)</span></Label>
            <Input
              value={label}
...
export default function PhoneNumbers() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Info banner */}
      <div className="rounded-lg border border-dash-border bg-dash-surface px-4 py-3 text-sm text-dash-t2 flex items-start gap-3">
            ...
            <tbody>
              {isLoading ? (
                ...
                numbers.map((num) => {
                  ...
                  return (
                    <tr key={num.id} className="border-b border-dash-border hover:bg-dash-surface transition-colors">
                      <td className="px-4 py-3 font-mono font-medium text-dash-t1">{num.phone_number}</td>
                      <td className="px-4 py-3 text-dash-t3">{num.label || '—'}</td>
                      <td className="px-4 py-3 text-dash-t1">{agent?.name || <span className="text-dash-t3">Unassigned</span>}</td>
