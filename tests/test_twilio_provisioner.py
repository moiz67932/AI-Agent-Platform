...
@pytest.mark.asyncio
async def test_create_livekit_routing_recovers_duplicate_dispatch_rule_using_existing_trunk_match(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    provisioner = provisioner_module.TwilioProvisioner(
        account_sid="AC123",
    ...
    captured: dict[str, object] = {"list_dispatch_rule_calls": 0}

    existing_rule = SimpleNamespace(
        sip_dispatch_rule_id="RULE123",
        name="stale-dispatch-name",
        trunk_ids=["TRUNK123"],
        hide_phone_number=False,
        inbound_numbers=[],
        metadata='{"agent_id":"agent-123","phone_number":"+13103410536"}',
        attributes={},
        room_preset="",
        krisp_enabled=False,
    ...
    class FakeSipApi:
        ...
        async def create_dispatch_rule(self, request):
            raise RuntimeError(
                "dispatch rule for same trunk, inbound number, number, and PIN combination already exists"
            )

        async def list_dispatch_rule(self, request):
            captured["list_dispatch_rule_calls"] = int(captured["list_dispatch_rule_calls"]) + 1
    ...
    assert captured["list_dispatch_rule_calls"] == 1
    assert captured["closed"] is True
